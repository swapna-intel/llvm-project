...
namespace wille {
  ...
  public:
  NumberExprAST(Location loc, int16_t val) : ExprAST(Expr_Num, loc), val(val) {}

    int16_t getVal() { return val; }
    /// LLVM style RTTI
  ...
  public:
    LiteralExprAST(Location loc, std::vector<std::unique_ptr<ExprAST>> values,
		   int64_t dim)
      :ExprAST(Expr_Literal, loc), values(std::move(values)),dim(dim) {}
    llvm::ArrayRef<std::unique_ptr<ExprAST>> getValues() { return values; }
    int64_t getDim() { return dim; }
  ...
  public:
  VarDeclExprAST(Location loc, llvm::StringRef name, std::unique_ptr<ExprAST> initVal)
    :ExprAST(Expr_VarDecl, loc), name(name), initVal(std::move(initVal)) {}
  ...
  /// This class represents the "prototype" for a function, which captures it's
  /// name, and it's argument names (thus implicitly the number of args)
  class PrototypeAST {
    Location location;
    std::string name;
  ...
  ///This class represents a fucntion definition itself.
  class FunctionAST {
